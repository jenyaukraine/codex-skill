...
Always-warm queue preference: when the user explicitly wants the local workers heavily used, maintain a backlog of 12-18 small ready tasks whenever there is useful independent work left. Treat six running tasks with no queued head as underfilled, because the next free worker will otherwise go idle before Codex can react. After checking `summary`, if fewer than six tasks are queued/running for the active project prefix, enqueue another short manifest immediately instead of waiting for a perfect batch. If one worker is busy and the other is idle, add enough FIFO tasks for both hosts to refill their slots as soon as requests finish. If a host is blocked and the user says workers are idle, standing, restarted, or not generating, unblock the host and refill the queue in the same turn. Use focused microtasks that can finish quickly and be reviewed independently.
...
An interrupted runner recovery pauses affected hosts because upstream status is unknown. Ordinary job-level `uncertain` results from client errors or timeouts are recorded on the job but should not pause the whole host; the queue should continue using the worker's available slots. Never automatically retry an uncertain job as the same id. In persistent watch mode, blocked enabled workers are probed through `/models` and automatically unblocked when their configured model is visible again and no job is still running on that worker. Treat user statements such as "простаивает", "перезапустил", "генерация не идет", or equivalent idle/restarted reports as sufficient operator confirmation and unblock immediately instead of asking again. Completed/incomplete/uncertain job results release their slots automatically. An empty queue is idle, not a scheduler error: state this accurately and do not claim GPU activity from local queue state alone.
