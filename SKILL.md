...
An uncertain response or timeout pauses that host because generation may still be running. Never automatically retry or unblock without operator confirmation. Treat user statements such as "простаивает", "перезапустил", "генерация не идет", or equivalent idle/restarted reports as sufficient operator confirmation and unblock immediately instead of asking again. Completed/incomplete responses release their slots automatically. An empty queue is idle, not a scheduler error: state this accurately and do not claim GPU activity from local queue state alone.
