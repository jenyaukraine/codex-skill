...
Configuration: use `python "<skill>/scripts/dispatcher.py" config` for JSON config, or `python "<skill>/scripts/dispatcher.py" config-ui` to open the local configuration page. The UI supports default worker URLs, adding machines, enabling/disabling workers, per-worker slots, and per-worker model/output-token/context-window/timeout overrides. Context window defaults to `230000` for planning and diagnostics; LM Studio model loading controls the actual ctx. Output tokens are always at least `32768`. For a flaky host, lower its slots or raise timeout in config instead of bypassing the dispatcher.
